	...
	def parse(self, tokens):
		...
		while token_index < len(tokens):
			...
			for token in tokens:
				...
				if token[0] == 'Start' and token[1] == 'program:':
				...
				elif token[0] == 'KEYWORD' and token[1] == 'EndFrom':
					parent = 'program'
					forb = False
					...
					ifb = True
					if_times +=1
				elif token[0] == 'KEYWORD' and token[1] == 'To':
					t = [tokens[token_index - 1][1],token[1],tokens[token_index + 1][1]]
					if forb == True:
						self.add_nood(parent,t)
				elif token[0] == 'KEYWORD' and token[1] == 'From':
					t = {'For':str(for_times)}
					self.add_nood(parent,t)
					parent = "For"+str(for_times)
					t = {'For'+str(for_times) : [[]]}
					self.parsed.append(t)
					forb = True
					for_times +=1
	...
	def add_func_node(self,parent,t):
		for dicta in self.parsed:
			for key,  value in dicta.items():
				if parent in key:
					dicta[parent].append(t)
	def add_for_node(self,parent,t):
		for dicta in self.parsed:
			for key,  value in dicta.items():
				if parent in key:
					dicta[parent][0].append(t)
	...
	def parse_for(self,k,v):
		...
		for node in self.object1:
			...
			for k2,v2 in node.items():
				# print(k2)
				if k2 == for_label:
					# print("FOUND KEY",k2,v2)
					self.parse_for_2(k2,v2)
	def parse_for_2(self,k,v):
		# print("LEVEL 2!",k,v)
		num_of_times = int(v[1][2]) - int(v[1][0])
		# print("NUM OF TIMES ",num_of_times)
		# print("Code to execute ",v[0])
		for token in v[0]:
			# print("TOKEN TO EXEC ",token)
			for k2,v2 in token.items():
				if k2 == 'Print':
					for i in range(num_of_times):
						self.parse_print(k2,v2)
				if k2 == 'Load':
					for i in range(num_of_times):
						self.parse_load(k2,v2)
				if k2 == 'Store':
					for i in range(num_of_times):
						self.parse_store(k2,v2)
						# print(i)
					# pass
